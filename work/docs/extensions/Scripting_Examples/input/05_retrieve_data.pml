[ch (id=retrieve_data) [title Retrieving Text And Data]
    ...
    This is cumbersome and error-prone, again. It's also a clear violation of the important [link (url=https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) Don't repeat yourself (DRY)] principle.
    ...
    [ch [title File]
        ...
        To retrieve a file's text content we can use function [link (url=[u:get PDML_ext_ref_manual_url]#fileUtils-readText ) fileUtils.readText].
    ...
    [ch [title External Program]
        ...
        We can call [link (url=[u:get PDML_ext_ref_manual_url]#OSCommand-textPipe ) OSCommand.textPipe] in a simple expression node to achieve this:
        [code
            ~~~
        ...
        [note
            ...
            Please refer to chapter [link (url=[u:get PDML_ext_user_manual_url]#script_nodes_syntax ) Syntax] for more information.
        ]

        A real-life example to demonstrate the power of executing external programs will be shown later in chapter [xref (ref_id=executing_OS_programs)].
    ]
    ...
    [ch [title OS Script]
        ...
        More examples about running OS scripts on Windows or Linux will be shown in [xref (ref_id=executing_OS_scripts)].
    ]
