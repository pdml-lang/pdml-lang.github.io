[ch (id=extensions) [title Extensions]
    ...
    As seen already, the syntax of [i Core PDML] is very simple and succinct - easy to read and write for humans and machines. Despite its simplicity, [i Core PDML] can be used to store small/big data/markup code.
...
[- TODO
    The documentation's [link (url=[u:get pdml_docs_url]index.html#extensions) Extensions] section provides more information about extensions that are part of PDML's specification.
-]
    ...
    [ch [title Character Escape Sequences]
        ...
        Besides the [link (url=[u:get pdml_docs_url]core/specification/index.html#escape_rules) mandatory] character escape sequences ([c \\\[], [c \\\]], and [c \\\\]), the following whitespace and Unicode escape sequences can be used:
        [sim_table
            ~~~
            Code, Description
            -
    ...
    [ch [title Types (work in progress)]
        ...
        Let's look at a real-world use-case of a PDML type in [link (url=https://www.pml-lang.dev/) PML]. Some PML nodes are designed to contain small or large pieces of raw text. For instance, PML has a node named [c code] to display highlighted source code. Suppose we want to show the following source code in a PML document:
        [code
            ~~~
            repeat 3 times
                write_line ( "[Hello]" )
        ...
        If we used only the [i Core PDML] syntax, and a [c code] node that is itself indented (because it's contained in a parent node), we would need to write:
        [code
            ~~~
                [code repeat 3 times
                write_line ( "\[Hello\]" )
